...
void Dialog::startThreads()
{
    ...
    /* Met à jour les champs de texte avec les nombres parsés (un zéro indiquera une erreur)
     * On passe par une QSting pour pouvoir afficher des long */
    ...
    if (nbIterations && nbThreads)
    {
        ...
        ui->lcdNumberResEspere->display(QString::number(nbIterations*nbThreads));
        ...
        /* Affiche le résultat obtenu (on passe par une QSting pour pouvoir afficher des long) */
        ui->lcdNumberResObtenu->display(QString::number(MyThread::compteur));
        ui->lcdNumberResRatio->display(QString::number(MyThread::compteur*100/nbIterations/nbThreads));
    }
