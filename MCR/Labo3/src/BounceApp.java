...
import Factory.RandomBorderFactory;
import Factory.RandomFullFactory;
		 ...
		 public void keyPressed(KeyEvent e) {
			switch (e.getKeyCode()) {
			   ...
			   case KeyEvent.VK_B: // Génère 10 cercles et 10 carres possédant une bordure.
				  for (int i = 0; i < 10; ++i) {
					 bouncers.add(RandomBorderFactory.getInstance().createCircle());
					 bouncers.add(RandomBorderFactory.getInstance().createSquare());
				  }
				  break;
			   case KeyEvent.VK_F: // générer 10 cercles et 10 pleins
				  for (int i = 0; i < 10; ++i) {
					 bouncers.add(RandomFullFactory.getInstance().createCircle());
					 bouncers.add(RandomFullFactory.getInstance().createSquare());
				  }
				  break;
