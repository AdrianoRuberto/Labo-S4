...
public class ControleurXMLCreation {
		...
		new Thread(){
			public void run(){
				...
				try {
					...
					globalDataControle();
					XMLSerializer serializer = new XMLSerializer("validator.dtd");
					writeToFile("data.xml", serializer, globalData);
				}
				catch(SAXException e){
					mainGUI.setErrorMessage("Validation XML impossible", e.toString());
				}
				catch (Exception e){
	...
	private static void writeToFile(String filename, XMLSerializer serializer, Object data) throws SAXException {
		try {
			BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(filename), "UTF-8"));
			serializer.toXML(data, out);
			out.close();
